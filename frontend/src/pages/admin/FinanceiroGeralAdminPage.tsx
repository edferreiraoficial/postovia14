...
export default function FinanceiroGeralAdminPage() {
  ...
  const gerarPdfResumoPeriodo = () => {
    ...
    const outrasReceitas = (resumoPeriodo.outrasReceitas || []).map((r: any) => `<tr><td>${escapar(dataBr(r.data))}</td><td>${escapar(r.descricao)}</td><td class="n ${classe(r.valor)}">${escapar(moedaPdf(r.valor))}</td></tr>`).join('');
  ...
  return <section className="financeiro-geral-page">
    ...
    <div ref={cabecalhoFlutuanteRef} className={`financeiro-geral-cabecalho-flutuante ${podeNumeroLancamento ? 'fg-com-numero' : 'fg-sem-numero'}`} aria-hidden="true"><table className="financeiro-geral-tabela"><colgroup><col className="fg-col-data" /><col className="fg-col-descricao" />{podeNumeroLancamento && <col className="fg-col-numero" />}<col className="fg-col-tipo-id" /><col className="fg-col-origem" />{colunasVisiveis.map(c => <col key={c.key} className={classeLarguraCampo(c)} />)}</colgroup><thead><tr><th className="fg-fixa-data" style={{position:'sticky',left:0,zIndex:130,background:'#ccd7e3'}}>Data</th><th className="fg-fixa-descricao" style={{position:'sticky',left:88,zIndex:129,background:'#ccd7e3'}}>Descrição</th>{podeNumeroLancamento && <th className="fg-fixa-numero" style={{position:'sticky',left:388,zIndex:128,background:'#ccd7e3'}}>Nº Lanc</th>}<th className="fg-fixa-tipo-id" style={{position:'sticky',left:podeNumeroLancamento ? 442 : 388,zIndex:127,background:'#ccd7e3',width:30,minWidth:30,maxWidth:30}}>T</th><th>Origem</th>{colunasVisiveis.map(c => <th key={c.key} className={c.key === 'total' ? 'fg-total' : ''}>{c.label}</th>)}</tr></thead></table></div>
    <div ref={tabelaWrapRef} className={`admin-card financeiro-geral-tabela-wrap ${podeNumeroLancamento ? 'fg-com-numero' : 'fg-sem-numero'}`}><table className="financeiro-geral-tabela"><colgroup><col className="fg-col-data" /><col className="fg-col-descricao" />{podeNumeroLancamento && <col className="fg-col-numero" />}<col className="fg-col-tipo-id" /><col className="fg-col-origem" />{colunasVisiveis.map(c => <col key={c.key} className={classeLarguraCampo(c)} />)}</colgroup>
      ...
      <tbody>{carregando ? <tr><td colSpan={4 + colunasVisiveis.length + (podeNumeroLancamento ? 1 : 0)}>Carregando...</td></tr> : linhas.map(l => <tr key={l.id} className={`${ehSaldo(l) ? 'fg-linha-saldo' : ''} ${!ehSaldo(l) ? 'fg-linha-editavel' : ''}`.trim()} onDoubleClick={() => abrirLancamentoParaEdicao(l)}>
        <td className="fg-fixa-data" style={{position:'sticky',left:0,zIndex:34}}>{dataBr(l.data_lancamento)}</td>
        <td className="fg-descricao fg-fixa-descricao" style={{position:'sticky',left:88,zIndex:33}}>{l.descricao_original || l.descricao_normalizada}</td>
        {podeNumeroLancamento && <td className="fg-numero-lancamento fg-fixa-numero" style={{position:'sticky',left:388,zIndex:32,width:54,minWidth:54,maxWidth:54}} title="Duplo clique para ajustar" onDoubleClick={(e) => { e.stopPropagation(); abrirAjusteNumero(l); }}>{l.id}</td>}
        <td className="fg-tipo-id fg-fixa-tipo-id" style={{position:'sticky',left:podeNumeroLancamento ? 442 : 388,zIndex:31,width:30,minWidth:30,maxWidth:30}}>{l.tipo_lancamento_id ?? ''}</td>
          ...
          return <>
            <div className="fg-detalhe-dia-lista fg-detalhe-dia-grade fg-resumo-periodo-grade">
              ...
              {(resumoPeriodo.outrasReceitas || []).length > 0 && <>
                ...
                {(resumoPeriodo.outrasReceitas || []).map((receita: any) => <div className="fg-resumo-periodo-despesa" key={`receita-${receita.id}`}><span>{dataBr(receita.data)}</span><span>{receita.descricao}</span><strong className={classeValor(receita.valor)}>{moeda(receita.valor)}</strong></div>)}
    ...
    {(detalheDia || carregandoDetalheDia) && <div className="fg-modal-overlay" role="dialog" aria-modal="true" aria-label="Resumo financeiro do dia">
      <div className="fg-modal fg-modal-detalhe-dia">
        ...
        {carregandoDetalheDia && !detalheDia ? <div className="fg-detalhe-carregando">Carregando...</div> : detalheDia && (() => {
          ...
          const totalOutrasReceitasDia = Number(detalheDia.totalOutrasReceitas || 0);
          const totalDespesasDia = Number(detalheDia.totalDespesas ?? (detalheDia.despesas || []).reduce((acc: number, despesa: any) => acc + Number(despesa.valor || 0), 0));
          const saldoFinalDia = Number(detalheDia.saldoDoDia ?? detalheDia.totalCalculado ?? detalheDia.totalComposicao ?? 0);
          const classeValor = (valor: any) => Number(valor || 0) < 0 ? 'fg-negativo' : '';
          return <div className="fg-detalhe-dia-lista fg-detalhe-dia-grade">
            ...
            {(detalheDia.outrasReceitas || []).length > 0 && <>
              <div className="fg-detalhe-subtitulo fg-detalhe-subtitulo-grade">Outras Receitas</div>
              {(detalheDia.outrasReceitas || []).map((receita: any) => <div className="fg-detalhe-grade-linha fg-detalhe-despesa" key={`receita-${receita.id}`}><span>{receita.descricao}</span><strong className={classeValor(receita.valor)}>{moeda(receita.valor)}</strong><span></span></div>)}
              <div className="fg-detalhe-grade-linha fg-detalhe-grade-total"><span>Total de Outras Receitas</span><span></span><strong className={classeValor(totalOutrasReceitasDia)}>{moeda(totalOutrasReceitasDia)}</strong></div>
            </>}
