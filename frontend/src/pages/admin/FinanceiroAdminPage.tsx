...
export default function FinanceiroAdminPage() {
  ...
  async function processar() {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/processar-financeiro-banco`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok) {
        ...
        const erro = tipoConteudo.includes('application/json')
          ? await response.json().catch(() => null)
          : null;
        throw new Error(erro?.erro || `Erro ao processar o período (HTTP ${response.status}).`);
      }
  ...
  return (
    <div className="admin-tool-page">
      ...
      <section className="admin-tool-card admin-single-upload-card">
        <div className="admin-tool-section-title">
          <div>
        ...
        <label className="admin-inline-file-row">
          <input
            type="file"
            accept=".xlsx,.xls"
