...
function extrairValorVendasDiaLmc(pagina, quantidadeVendas = null, precoVenda = null) {
  ...
  // O campo 10.1 é o VALOR DE VENDAS DO DIA. Em alguns PDFs o pdf-parse/OCR
  // junta o campo 10.1 com o campo 10.2 (acumulado do mês), fazendo com que
  // a extração por trecho pegue o acumulado. Por isso usamos a venda do dia
  // calculada por litros vendidos x preço unitário como trava de segurança.
  const valorCalculadoDia = quantidade > 0 && preco > 0
    ? arredondarCentavos(quantidade * preco)
    : null

  const trecho = (texto.match(/10\.1\)\s*Valor de vendas do dia([\s\S]*?)(?:10\.2\)|10\.2\s*\))/i) || [])[1] || ''
  const valores = numerosDoTexto(trecho).filter(v => v !== null)
  const valorExtraido = valores.length ? valores.at(-1) : null

  if (valorCalculadoDia !== null) {
    // Se o valor extraído for muito diferente do cálculo, normalmente é porque
    // o parser pegou o 10.2 (acumulado). Nesses casos, grava o 10.1 correto.
...
function extrairLinhaLmc(pagina) {
  ...
  const valorVendasCampo101 = extrairValorVendasDiaLmc(pagina, consolidado.quantVendas, precoVenda)
  const valorVendas = valorVendasCampo101 !== null
    ? valorVendasCampo101
    : arredondarCentavos(Math.abs(Number(consolidado.quantVendas || 0)) * Number(precoVenda || 0))
